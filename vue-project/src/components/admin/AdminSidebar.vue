...
<template>
    ...
    <v-navigation-drawer permanent location="left" width="250" class="sidebar">
        ...
        <v-list nav dense class="ml-2 custom-font">
            <v-list-item prepend-icon="mdi-home-outline" title="Dashboard" :to="{ name: 'AdminDashboard' }" />
            <v-list-item prepend-icon="mdi-account" title="User Accounts" :to="{ name: 'UserManagement' }" />
            <v-list-item prepend-icon="mdi-account-group    " title="Resident Profile" :to="{ name: 'ResidentManagement' }" />
            ...
            <v-list-item prepend-icon="mdi-message-alert" title="Complaints" :to="{ path: '/admin/record-complaint' }" />
        </v-list>
