<?php
// api/endpoints/read_reservations_by_facility.php
...
// Get facility from URL parameters
$facility = isset($_GET['facility']) ? $_GET['facility'] : '';

if (empty($facility)) {
    http_response_code(400);
    echo json_encode(array('message' => 'Facility parameter is required'));
    exit();
}

// Get reservations by facility
$result = $reservation->getByFacility($facility);
$num = $result->rowCount();
...
if ($num > 0) {
    ...
    echo json_encode(array('message' => 'No reservations found for facility: ' . $facility));
}
?>
